...
ENT.Type = "anim"
ENT.Base = "base_anim"
ENT.PrintName = "Incendiary Grenades"
...
if CLIENT then
	...
	local Text = "Incendiary Grenades"
	...
	function ENT:Draw()
		...
		cam.Start3D2D(self:GetPos() + (self:GetUp() * 10) + (self:GetRight() * 5) + (self:GetForward() * 17),FixAngles,0.13)
...
elseif SERVER then -- Make sure CS:GO is mounted!
	...
	function ENT:Use(Activator)
		if Activator:IsPlayer() then
			if Activator:GetWeapon("m9k_mmm_incendiary") == NULL then
				Activator:Give("m9k_mmm_incendiary")
				Activator:GiveAmmo(5,"m9k_mmm_incendiary")
				Activator:SelectWeapon("m9k_mmm_incendiary") -- Has no effect in multiplayer in this case but is required in singleplayer!
			else
				Activator:GiveAmmo(6,"m9k_mmm_incendiary")
			end
	...
	function ENT:Splode()
		...
		if self.iHealth <= 0 and not self.Sploded then
			...
			for I = 1,6 do
				local DroppedEnt = ents.Create("m9k_mmm_incendiary")
