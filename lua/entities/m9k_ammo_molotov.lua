...
ENT.Type = "anim"
ENT.Base = "base_anim"
ENT.PrintName = "Molotov Crate (CS:GO)"
...
elseif SERVER then -- Make sure CS:GO is mounted!
	...
	function ENT:Use(Activator)
		if Activator:IsPlayer() then
			if Activator:GetWeapon("m9k_mmm_molotov") == NULL then
				...
				Activator:SelectWeapon("m9k_mmm_molotov") -- Has no effect in multiplayer in this case but is required in singleplayer!
			else
	...
	function ENT:Splode()
		...
		if self.iHealth <= 0 and not self.Sploded then
			...
			for I = 1,6 do
				local DroppedEnt = ents.Create("m9k_mmm_molotov")
