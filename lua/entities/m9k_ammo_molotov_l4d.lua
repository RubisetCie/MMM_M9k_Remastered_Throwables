...
ENT.Type = "anim"
ENT.Base = "base_anim"
ENT.PrintName = "Molotov Crate (L4D)"
...
if not IsMounted("left4dead") and not IsMounted("left4dead2") then
	...
	return
elseif SERVER then -- We make sure that either Left4Dead or Left4Dead2 is installed since the models are identical.
	...
	function ENT:Use(Activator)
		if Activator:IsPlayer() then
			if Activator:GetWeapon("m9k_mmm_molotov") == NULL then
				Activator:Give("m9k_mmm_molotov_l4d")
				Activator:GiveAmmo(5,"m9k_mmm_molotov")
				Activator:SelectWeapon("m9k_mmm_molotov_l4d") -- Has no effect in multiplayer in this case but is required in singleplayer!
			else
				Activator:GiveAmmo(6,"m9k_mmm_molotov")
			end
	...
	function ENT:Splode()
		...
		if self.iHealth <= 0 and not self.Sploded then
			...
			for I = 1,6 do
				local DroppedEnt = ents.Create("m9k_mmm_molotov_l4d")
